...
def create_experiment(**kwargs):
    ...
    ## TODO: There is a bug here with experiement tracking in mlflow. Deleting previously experiments via command line does not work, only work around is to delete the mlflow.db, but that comes with restarting docker container. The current hack is to increase the experiment number every run
    ## TODO: check if experiment exists, delete, then create again?
    experiment_id = mlflow.create_experiment("intel_scenes_compare_models8")
...
def register_best_model(**kwargs):
    ...
    task_instance = kwargs['ti']
    task_instance_data = task_instance.xcom_pull(task_ids='eval_models')
    experiment_id = task_instance_data[0]
